...
결과: [1, 2, 3, 4, 5, 6]`}</code></pre>
              ...
              <div className="callout-box" data-aos="fade-up">
                <strong>핵심 관찰:</strong> 현재 요소(key)를 정렬된 부분에서 올바른 위치에 "삽입"합니다.
                이미 정렬된 데이터에서는 비교만 하고 이동이 거의 없으므로 O(n)에 가깝게 동작합니다.
              </div>
            </section>
...
# 같은 키 3: A가 C보다 앞 (원래 순서 유지) ✓`}</code></pre>
              ...
              <div className="callout-box" data-aos="fade-up">
                <strong>왜 안정 정렬이 중요한가?</strong> 예를 들어 학생 목록을 먼저 이름순으로 정렬한 후
                성적순으로 다시 정렬할 때, 안정 정렬이면 같은 성적의 학생들은 이름순이 유지됩니다.
              </div>
            </section>
...
insertion_sort_verbose([5, 2, 4, 6, 1, 3])`}</code></pre>
      ...
      <nav className="lesson-nav" data-aos="fade-up">
        <Link to="/algorithm/selection-sort" className="lesson-nav-prev">
          <span className="lesson-nav-label">이전</span>
          <span className="lesson-nav-title">선택 정렬</span>
        </Link>
        <Link to="/algorithm/merge-sort" className="lesson-nav-next">
